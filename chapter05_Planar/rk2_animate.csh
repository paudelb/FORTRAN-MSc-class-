...
if( $findrange )then #determine x-y range by extreme values:
 set xy = (`awk 'BEGIN{x=1e200;X=-x;y=x;Y=X}{if($2<x){x=$2};if($2>X){X=$2};if($3<y){y=$3};if($3>Y){Y=$3};}END{dx=0.03*(X-x);dy=0.03*(Y-y);print x-dx,X+dx,y-dy,Y+dy}' $file`)
...
plot $xyrange "<cat -n $file"  \
  using 3:(\$1<= icount ? \$4: 1/0) with lines notit
$tdelay
