...
program dlo_solve
 ...
 integer :: Nt, i,Ntrans,NPIs,NtPI
 real(8) :: da0,PI


 NtPI = 200;  !how many intervals in the interval [t,t+PI]
 NPIs = 20000 !total time is NPIs*PI
 Nt   = NtPI*NPIs+1
 PI   = atan2(0.0D0,-1.0D0); 
 Ti   = 0.0D0; Tf = Ti+NPIs*PI; Ntrans   = 10000;
 omega_0  = 1.0D0             ; omega    = 2.0D0;  gamma = 0.2D0
 omega_02 = omega_0*omega_0   ; omega2   = omega*omega
 X10      = 3.1D0             ; X20      = 0.0D0 
 print *,'# PI,NtPI,NPIs,Nt= ',PI,NtPI,NPIs,Nt
 print *,'# Ti,Tf,Ntrans,omega_0,omega,gamma,X10,X20= ',Ti,Tf,Ntrans,omega_0,omega,gamma,X10,X20
 open(unit=11,file='fdp-poincare.dat')

 !Set the value of the force amplitude A here:
 a_0 = 4.450D0; print *,'# A= ',a_0
 call RK(T,X1,X2,Ti,Tf,X10,X20,Nt)
 do i=Ntrans+1,Nt,NtPi
  write(11,*) a_0, X1(i),X2(i)
 enddo
